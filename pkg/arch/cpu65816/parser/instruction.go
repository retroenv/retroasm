...
func parseInstruction(parser arch.Parser, instructionDetails *cpu65816.Instruction) (ast.Node, error) {
	...
	if next1.Type == token.Comma {
		...
		if ins.instruction.HasAddressing(cpu65816.BlockMoveAddressing) {
	...
	case ins.arg1.Type == token.Identifier || ins.instruction.HasAddressing(cpu65816.AccumulatorAddressing) || ins.arg1.Type.IsTerminator():
...
func parseInstructionBracketSingle(ins *instruction) (ast.Node, error) {
	...
	if ins.instruction.HasAddressing(cpu65816.AbsoluteIndirectLongAddressing) {
		return ast.NewInstruction(ins.instruction.Name, int(cpu65816.AbsoluteIndirectLongAddressing), argument, ins.modifiers), nil
	}
	if ins.instruction.HasAddressing(cpu65816.DirectPageIndirectLongAddressing) {
		return ast.NewInstruction(ins.instruction.Name, int(cpu65816.DirectPageIndirectLongAddressing), argument, ins.modifiers), nil
	}
...
func parseInstructionBracketIndexed(ins *instruction) (ast.Node, error) {
	...
	if ins.instruction.HasAddressing(cpu65816.DirectPageIndirectLongIndexedYAddressing) {
		return ast.NewInstruction(ins.instruction.Name, int(cpu65816.DirectPageIndirectLongIndexedYAddressing), argument, ins.modifiers), nil
	}
...
func parseInstructionParenSingle(parser arch.Parser, ins *instruction) (ast.Node, error) {
	...
	if ins.instruction.HasAddressing(cpu65816.DirectPageIndirectAddressing) ||
		ins.instruction.HasAddressing(cpu65816.AbsoluteIndirectAddressing) {
		...
		if ins.instruction.HasAddressing(cpu65816.DirectPageIndirectAddressing) {
			return ast.NewInstruction(ins.instruction.Name, int(cpu65816.DirectPageIndirectAddressing), argument, ins.modifiers), nil
		}
		return ast.NewInstruction(ins.instruction.Name, int(cpu65816.AbsoluteIndirectAddressing), argument, ins.modifiers), nil
	}
...
func parseStackRelativeParentheses(parser arch.Parser, ins *instruction) (ast.Node, error) {
	...
	if next.Type != token.Comma {
		...
		if ins.instruction.HasAddressing(cpu65816.StackRelativeAddressing) {
			return ast.NewInstruction(ins.instruction.Name, int(cpu65816.StackRelativeAddressing), argument, ins.modifiers), nil
		}
	...
	if ins.instruction.HasAddressing(cpu65816.StackRelativeIndirectIndexedYAddressing) {
		return ast.NewInstruction(ins.instruction.Name, int(cpu65816.StackRelativeIndirectIndexedYAddressing), argument, ins.modifiers), nil
	}
...
func parseInstructionSingleIdentifier(parser arch.Parser, ins *instruction) (ast.Node, error) {
	if cpu65816.BranchingInstructions.Contains(ins.instruction.Name) {
		return parseBranchingInstruction(parser, ins)
	}

	if ins.instruction.HasAddressing(cpu65816.AccumulatorAddressing) {
	...
	case addressingAbsolute:
		if !ins.instruction.HasAddressing(cpu65816.AbsoluteAddressing) {
	...
	case addressingDirectPage:
		if !ins.instruction.HasAddressing(cpu65816.DirectPageAddressing) {
			return nil, errors.New("invalid direct page addressing mode usage")
		}
		addressing = cpu65816.DirectPageAddressing

	case addressingLong:
		if !ins.instruction.HasAddressing(cpu65816.AbsoluteLongAddressing) {
			return nil, errors.New("invalid long addressing mode usage")
		}
		addressing = cpu65816.AbsoluteLongAddressing
...
func resolveDefaultAddressing(ins *instruction) cpu65816.AddressingMode {
	hasAbsolute := ins.instruction.HasAddressing(cpu65816.AbsoluteAddressing)
	hasDirectPage := ins.instruction.HasAddressing(cpu65816.DirectPageAddressing)
...
func parseInstructionSingleIdentifierAccumulator(parser arch.Parser, ins *instruction) ast.Node {
	...
	return ast.NewInstruction(ins.instruction.Name, int(cpu65816.AccumulatorAddressing), nil, ins.modifiers)
}
...
func parseBranchingInstruction(parser arch.Parser, ins *instruction) (ast.Node, error) {
	...
	switch {
	case ins.instruction.HasAddressing(cpu65816.RelativeAddressing):
		addressing = cpu65816.RelativeAddressing
	case ins.instruction.HasAddressing(cpu65816.RelativeLongAddressing):
		addressing = cpu65816.RelativeLongAddressing
	case ins.instruction.HasAddressing(cpu65816.AbsoluteAddressing):
		addressing = cpu65816.AbsoluteAddressing
	case ins.instruction.HasAddressing(cpu65816.AbsoluteLongAddressing):
		addressing = cpu65816.AbsoluteLongAddressing
	default:
...
func parseBranchingIndirect(parser arch.Parser, ins *instruction) (ast.Node, error) {
	...
	if param.Type == token.Comma {
		...
		if !ins.instruction.HasAddressing(cpu65816.AbsoluteIndexedXIndirectAddressing) {
			return nil, errors.New("instruction does not support absolute indexed X indirect addressing")
		}

		l := ast.NewLabel(ins.arg1.Value)
		return ast.NewInstruction(ins.instruction.Name, int(cpu65816.AbsoluteIndexedXIndirectAddressing), l, nil), nil
	}
	...
	if !ins.instruction.HasAddressing(cpu65816.AbsoluteIndirectAddressing) {
		return nil, errors.New("instruction does not support indirect addressing")
	}

	l := ast.NewLabel(ins.arg1.Value)
	return ast.NewInstruction(ins.instruction.Name, int(cpu65816.AbsoluteIndirectAddressing), l, nil), nil
}
...
func parseBranchingIndirectLong(parser arch.Parser, ins *instruction) (ast.Node, error) {
	...
	l := ast.NewLabel(ins.arg1.Value)
	return ast.NewInstruction(ins.instruction.Name, int(cpu65816.AbsoluteIndirectLongAddressing), l, nil), nil
}
...
func parseBlockMove(parser arch.Parser, ins *instruction) (ast.Node, error) {
	...
	return ast.NewInstruction(ins.instruction.Name, int(cpu65816.BlockMoveAddressing), n, nil), nil
}
...
func parseInstructionNumberParameter(ins *instruction) (ast.Node, error) {
	...
	addressing := cpu65816.NoAddressing
...
func resolveDefaultNumberAddressing(ins *instruction, value uint64) cpu65816.AddressingMode {
	switch {
	case value <= math.MaxUint8 && ins.instruction.HasAddressing(cpu65816.DirectPageAddressing):
		return cpu65816.DirectPageAddressing
	case value <= math.MaxUint16 && ins.instruction.HasAddressing(cpu65816.AbsoluteAddressing):
		return cpu65816.AbsoluteAddressing
	case ins.instruction.HasAddressing(cpu65816.AbsoluteLongAddressing):
		return cpu65816.AbsoluteLongAddressing
	case ins.instruction.HasAddressing(cpu65816.AbsoluteAddressing):
		return cpu65816.AbsoluteAddressing
	case ins.instruction.HasAddressing(cpu65816.DirectPageAddressing):
		return cpu65816.DirectPageAddressing
	default:
		return cpu65816.NoAddressing
	}
