...
func TestArchitectureLookup(t *testing.T) {
	...
	tests := []struct {
		...
		{"clc", cpu65816.ClcName},
		{"lda", cpu65816.LdaName},
		{"nop", cpu65816.NopName},
		{"rtl", cpu65816.RtlName},
		{"rts", cpu65816.RtsName},
		{"sta", cpu65816.StaName},
		{"xba", cpu65816.XbaName},
		{"xce", cpu65816.XceName},
	}
...
func TestAssembleImpliedInstructions(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			result := assembleCPU65816(t, tt.asm)
...
RTS
`
	result := assembleCPU65816(t, source)
...
func TestAssembleAST(t *testing.T) {
	cfg := New()
	assert.NoError(t, cfg.ReadCa65Config(bytes.NewReader([]byte(defaultCPU65816Config))))
...
func assembleCPU65816(t *testing.T, source string) []byte {
	t.Helper()
