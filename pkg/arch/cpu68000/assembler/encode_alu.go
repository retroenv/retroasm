...
func encodeMOVE(assigner arch.AddressAssigner, resolved parser.ResolvedInstruction) ([]byte, error) {
	var sizeBits uint16
	...
	if resolved.Instruction.Name == cpu68000.MOVEAName || dstMode == 1 {
		dstMode = 1
	}
...
func encodeAddSub(assigner arch.AddressAssigner, resolved parser.ResolvedInstruction, baseOpcode uint16) ([]byte, error) {
	...
	// ADDA/SUBA
	if name == cpu68000.ADDAName || name == cpu68000.SUBAName {
		return encodeAddrRegOp(assigner, resolved, baseOpcode)
	}

	// ADDX/SUBX
	if name == cpu68000.ADDXName || name == cpu68000.SUBXName {
...
func encodeExtendedOp(resolved parser.ResolvedInstruction, baseOpcode uint16) ([]byte, error) {
...
func encodeEOR(assigner arch.AddressAssigner, resolved parser.ResolvedInstruction) ([]byte, error) {
	// EOR Dn,<ea>
...
func encodeCMP(assigner arch.AddressAssigner, resolved parser.ResolvedInstruction) ([]byte, error) {
	name := resolved.Instruction.Name

	if name == cpu68000.CMPAName {
...
func encodeCMPM(resolved parser.ResolvedInstruction) ([]byte, error) {
