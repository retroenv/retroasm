...
func encodeScc(assigner arch.AddressAssigner, resolved parser.ResolvedInstruction) ([]byte, error) {
	cond := resolved.Extra
...
func encodeMOVEQ(assigner arch.AddressAssigner, resolved parser.ResolvedInstruction) ([]byte, error) {
...
func encodeLEA(assigner arch.AddressAssigner, resolved parser.ResolvedInstruction) ([]byte, error) {
	...
	buf := encodeWord(opcode)
	buf, err := appendEAExtensionWords(buf, assigner, resolved.SrcEA, cpu68000.SizeLong)
	if err != nil {
		return nil, err
	}
	return buf, nil
...
func encodePEA(assigner arch.AddressAssigner, resolved parser.ResolvedInstruction) ([]byte, error) {
	dstMode, dstReg := encodeEAField(resolved.DstEA)

	opcode := uint16(0x4840) | uint16(dstMode)<<3 | uint16(dstReg)
	buf := encodeWord(opcode)
	buf, err := appendEAExtensionWords(buf, assigner, resolved.DstEA, cpu68000.SizeLong)
	if err != nil {
		return nil, err
	}
	return buf, nil
...
func encodeCHK(assigner arch.AddressAssigner, resolved parser.ResolvedInstruction) ([]byte, error) {
	dn := uint16(resolved.DstEA.Register)
	srcMode, srcReg := encodeEAField(resolved.SrcEA)

	opcode := uint16(0x4180) | dn<<9 | uint16(srcMode)<<3 | uint16(srcReg)
	buf := encodeWord(opcode)
	buf, err := appendEAExtensionWords(buf, assigner, resolved.SrcEA, cpu68000.SizeWord)
	if err != nil {
		return nil, err
	}
	return buf, nil
...
func encodeSWAP(resolved parser.ResolvedInstruction) ([]byte, error) {
	dn := uint16(resolved.DstEA.Register)
	return encodeWord(0x4840 | dn), nil
}

func encodeEXT(resolved parser.ResolvedInstruction) ([]byte, error) {
	dn := uint16(resolved.DstEA.Register)
	if resolved.Size == cpu68000.SizeLong {
		return encodeWord(0x48C0 | dn), nil
	}
	return encodeWord(0x4880 | dn), nil
}

func encodeEXG(resolved parser.ResolvedInstruction) ([]byte, error) {
	rx := uint16(resolved.SrcEA.Register)
	ry := uint16(resolved.DstEA.Register)

	srcIsAddr := resolved.SrcEA.Mode == cpu68000.AddrRegDirectMode
	dstIsAddr := resolved.DstEA.Mode == cpu68000.AddrRegDirectMode
...
func encodeLINK(assigner arch.AddressAssigner, resolved parser.ResolvedInstruction) ([]byte, error) {
...
func encodeUNLK(resolved parser.ResolvedInstruction) ([]byte, error) {
	an := uint16(resolved.DstEA.Register)
	return encodeWord(0x4E58 | an), nil
}

func encodeTRAP(assigner arch.AddressAssigner, resolved parser.ResolvedInstruction) ([]byte, error) {
	v, err := eaValue(assigner, resolved.SrcEA)
	if err != nil {
		return nil, err
	}
...
func encodeSTOP(assigner arch.AddressAssigner, resolved parser.ResolvedInstruction) ([]byte, error) {
	v, err := eaValue(assigner, resolved.SrcEA)
	if err != nil {
		return nil, err
	}
...
func encodeMOVEM(assigner arch.AddressAssigner, resolved parser.ResolvedInstruction) ([]byte, error) {
	var szBit uint16
	if resolved.Size == cpu68000.SizeLong {
		szBit = 1
	}
	...
	if resolved.Extra == 0 {
		...
		if resolved.DstEA.Mode == cpu68000.PreDecrementMode {
...
func encodeMOVEP(assigner arch.AddressAssigner, resolved parser.ResolvedInstruction) ([]byte, error) {
	var dn, an uint16
	var dir uint16

	if resolved.SrcEA.Mode == cpu68000.DataRegDirectMode {
		// MOVEP Dn,d16(An)
		dn = uint16(resolved.SrcEA.Register)
		an = uint16(resolved.DstEA.Register)
		dir = 1
	...
	// Append displacement
	var dispEA *parser.EffectiveAddress
	if dir == 1 {
...
func encodeBitOp(assigner arch.AddressAssigner, resolved parser.ResolvedInstruction, bitOp uint16) ([]byte, error) {
	dstMode, dstReg := encodeEAField(resolved.DstEA)
...
func encodeBCD(resolved parser.ResolvedInstruction, baseOpcode uint16) ([]byte, error) {
	rx := uint16(resolved.DstEA.Register)
	ry := uint16(resolved.SrcEA.Register)

	var rm uint16
	if resolved.SrcEA.Mode == cpu68000.PreDecrementMode {
		rm = 1
	}
...
func encodeShiftRotate(assigner arch.AddressAssigner, resolved parser.ResolvedInstruction) ([]byte, error) {
	...
	if resolved.DstEA != nil && resolved.SrcEA == nil {
		...
		buf := encodeWord(opcode)
		buf, err := appendEAExtensionWords(buf, assigner, resolved.DstEA, cpu68000.SizeWord)
		if err != nil {
			return nil, err
		}
		return buf, nil
...
func shiftTypeAndDirection(name string) (uint16, uint16) {
	switch name {
	case cpu68000.ASRName:
		return 0, 0
	case cpu68000.ASLName:
		return 0, 1
	case cpu68000.LSRName:
		return 1, 0
	case cpu68000.LSLName:
		return 1, 1
	case cpu68000.ROXRName:
		return 2, 0
	case cpu68000.ROXLName:
		return 2, 1
	case cpu68000.RORName:
		return 3, 0
	case cpu68000.ROLName:
		return 3, 1
	default:
		return 0, 0
	}
