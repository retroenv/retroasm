...
// ParseIdentifier parses an CPU68000 instruction and returns the corresponding AST node.
func ParseIdentifier(p arch.Parser, ins *cpu68000.Instruction, mnemonic string) (ast.Node, error) { //nolint:cyclop,funlen // instruction dispatch requires many cases
	size := cpu68000.SizeWord // default size
	...
	case ins.Name == cpu68000.DBccName:
		err = parseDBcc(p, resolved)
	case ins.Name == cpu68000.MOVEMName:
		err = parseMOVEM(p, resolved)
	case ins.Name == cpu68000.MOVEQName:
		err = parseMOVEQ(p, resolved)
	case ins.Name == cpu68000.TRAPName:
		err = parseTRAP(p, resolved)
	case ins.Name == cpu68000.STOPName:
		err = parseSTOP(p, resolved)
	case ins.Name == cpu68000.LINKName:
		err = parseLINK(p, resolved)
	case ins.Name == cpu68000.UNLKName:
		err = parseUNLK(p, resolved)
	case ins.Name == cpu68000.SWAPName || ins.Name == cpu68000.EXTName:
		err = parseDataRegOnly(p, resolved)
	case ins.Name == cpu68000.EXGName:
...
func buildInstructionNode(name string, resolved *ResolvedInstruction) ast.Node {
	...
	return ast.NewInstruction(name, int(cpu68000.NoAddressing), argument, nil)
}
...
func isBranchInstruction(name string) bool {
	return name == cpu68000.BccName || name == cpu68000.BRAName || name == cpu68000.BSRName
}

func isQuickInstruction(name string) bool {
	return name == cpu68000.ADDQName || name == cpu68000.SUBQName
}
...
func parseMOVEQ(p arch.Parser, resolved *ResolvedInstruction) error {
	...
	resolved.DstEA = dst
	resolved.Size = cpu68000.SizeLong
	return nil
}
...
func parseQuick(p arch.Parser, resolved *ResolvedInstruction) error {
	...
	if src != nil && src.Mode == cpu68000.ImmediateMode {
		src.Mode = cpu68000.QuickImmediateMode
	}
	resolved.SrcEA = src
