...
func TestArchitectureLookup(t *testing.T) {
	...
	tests := []struct {
		...
		{"nop", cpu68000.NOPName},
		{"rts", cpu68000.RTSName},
		{"move", cpu68000.MOVEName},
		{"move.l", cpu68000.MOVEName},
		{"move.w", cpu68000.MOVEName},
		{"move.b", cpu68000.MOVEName},
		{"add", cpu68000.ADDName},
		{"beq", cpu68000.BccName},
		{"bne", cpu68000.BccName},
		{"bra", cpu68000.BRAName},
		{"dbne", cpu68000.DBccName},
		{"shi", cpu68000.SccName},
		{"lea", cpu68000.LEAName},
		{"movea", cpu68000.MOVEAName},
		{"movem", cpu68000.MOVEMName},
	}
...
func TestAssembleSimpleInstructions(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			result := assembleCPU68000(t, tt.asm)
...
RTS
`
	result := assembleCPU68000(t, source)
	// MOVEQ #0,D0 = 0x7000
	// NOP = 0x4E71
	// RTS = 0x4E75
...
func TestAssembleAST(t *testing.T) {
	cfg := New()
	assert.NoError(t, cfg.ReadCa65Config(bytes.NewReader([]byte(defaultCPU68000Config))))
...
func assembleCPU68000(t *testing.T, source string) []byte {
	t.Helper()
